...
export class Crypto implements OnInit {
  ...
  async buyCoin(coin: any): Promise<void> {
    if (!this.userId || this.userId <= 0) {
      this.uiService.alert('You must be logged in to buy crypto.');
      return;
    }
  ...
  addToFavourite(coin: any): void {
    if (!this.userId || this.userId <= 0) {
      this.uiService.alert('You must be logged in first.');
      return;
    }
    ...
    this.crypto.addToFavorites(payload).subscribe({
      next: () => {
        this.uiService.alert(`${coin.cryptoName} added to favourites`);
      },
      error: (err) => {
        console.error('Favourite failed', err);
        console.error('Favourite error body', err.error);
        this.uiService.alert(err?.error?.message || 'Failed to add favourite');
      }
    });
  }
