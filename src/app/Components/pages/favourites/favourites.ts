...
export class Favourites implements OnInit {
  ...
  constructor(private crypto: CryptoService, private uiService: UiService) {}
 ...
 ngOnInit(): void {
  ...
  removeFavourite(item: any): void {
    this.crypto.removeFavorite(item.fid).subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.uiService.alert('Failed to remove favourite');
      }
    });
  }

  async buyCoin(item: any): Promise<void> {
    ...
    if (isNaN(amount) || amount <= 0) {
      this.uiService.alert('Please enter valid amount');
      return;
    }
